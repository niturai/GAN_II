...
The Python files follow the information of the model of the star and baseline configuration. There is need to run these scripts one by one in given order below...

## To create N number of Images....
Run generate_image.py

The parameters can be changed using these two scripts
...
The parameters can be changed using these four scripts
...
Run generate_loss.py for the visualization of loss function for two different parameters. The loss is stored in "logs.pkl" file.
              
## Train the model with GAN for the given signal
Run main_def_test.py to check the definition.
Run gan_def_test.py to train the model.

The parameters can be changed using these three scripts
1. main_def.py: This script prepares the set of definitions used for the GAN structure while training.                           
2. gan_def, py: This script describes the GAN network as a generator and a Discriminator. Using these networks, GAN can train the model.
3. gan_def_test.py: This script test the "gan_def.py" and model is trained here.

## Visualization of trained result
Run generate_reconst.py

## Statistical Visualization of Result
Run generate_moment.py

The definition of moments is defined in "moment.py"
